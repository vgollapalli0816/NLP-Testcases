...
MODEL_URLS = {
    "model1": "https://raw.githubusercontent.com/vgollapalli0816/NLP-Testcases/main/knn_model_expected.joblib",
    "model2": "https://raw.githubusercontent.com/vgollapalli0816/NLP-Testcases/main/knn_model_steps.joblib",
    "model3": "https://raw.githubusercontent.com/vgollapalli0816/NLP-Testcases/main/tfidf_vectorizer.pkl",
}

# Function to download and load a model
@st.cache_data
def load_model(url, file_format):
    with urllib.request.urlopen(url) as response:
        content = response.read()
        if file_format == "pkl":
            return joblib.load(io.BytesIO(content))  # Use joblib for better compatibility
        elif file_format == "joblib":
            return joblib.load(io.BytesIO(content))

# Load all models into variables
knn_model_results = load_model(MODEL_URLS["model1"], "joblib")
knn_model_steps = load_model(MODEL_URLS["model2"], "joblib")
tfidf_vectorizer = load_model(MODEL_URLS["model3"], "pkl")
...
def generate_output(acceptance_criteria, model, vectorizer):
    ...
    print(acceptance_criteria_vector.shape)
